...
export class AmqpModule implements OnModuleDestroy {
    ...
    private static createConnectionProvider(options: AmqpOptionsInterface): Provider {
        ...
        return {
            ...
            useFactory: async (config: AmqpOptionsInterface) => await from(amqp.connect((config as any)[`${options.name ? options.name : 'default'}`])).pipe(retry(options.retrys, options.retryDelay)).toPromise(),
